...
;(function (angular) {
    ...
    angular.module('core').run(function ($rootScope, $state, $window, AuthService) {
        'ngInject';

        var DEFAULT_PAGE = 'app.auth.overview';
        var LOGIN_PAGE = 'app.non_auth.login';

        $rootScope.go = $state.go;

        $rootScope.$on('$stateChangeStart', function () {
            var stateAuth = arguments[1].name.split('.')[1];
